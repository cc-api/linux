...
print_task(struct seq_file *m, struct rq *rq, struct task_struct *p)
{
	...
	SEQ_printf(m, "%15s %5d %2d %9Ld.%06ld %c %9Ld.%06ld %9Ld.%06ld %9Ld.%06ld %9Ld %5d ",
...
static void dump_sd_properties(struct seq_file *m, int cpu)
{
	...
	SEQ_printf(m, "Highest domain with asym_packing: %s\n", sd ? sd->name : "null");

	sd = rcu_dereference(per_cpu(sd_llc, cpu));
	SEQ_printf(m, "Highest domain at sd_llc %s\n", sd ? sd->name : "null");
