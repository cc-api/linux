				   ...
				   struct swap_tasks_arg *arg)
{
	...
	long throughput_xchg, throughput_no_xchg;
	...
	list_for_each_entry_reverse(p_src,
		...
		/*
		 * TODO: Skip unclassified tasks, but the scheduler does not
		 * understand p_src->ipcc.
		 */
...
static void init_rq_ipcc_stats(struct sg_lb_stats *sgs)
{
	...
	sgs->min_score = LONG_MAX;
}

static int rq_last_task_ipcc(int dst_cpu, struct rq *rq, unsigned int *ipcc)
{
				     ...
				     struct rq *rq)
{
	...
	/*
	 * Invalidate the statistics of this group so that they are not
	 * considered later when finding the busiest group.
	 */
	if (score < 0) {
		sgs->sum_score = -EINVAL;
		return;
	}
				      ...
				      struct rq *rq)
{
	...
	if (score_on_dst_cpu > score_on_src_cpu)
		sgs->ipcc_score_after = max(sgs->ipcc_score_after,
					    score_on_dst_cpu);
}
				     ...
				     struct sched_group *group)
{
...
#ifdef CONFIG_IPC_CLASSES
	long busiest_score_on_dst_cpu = 0;
#endif
	int i;
	...
	for_each_cpu_and(i, sched_group_span(group), env->cpus) {
		 ...
		 * ipcc_score_delta()?
		 */
		long score_on_dst_cpu, score_on_src_cpu;
#endif
