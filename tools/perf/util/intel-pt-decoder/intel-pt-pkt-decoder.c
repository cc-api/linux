		      ...
		      size_t buf_len)
{
	...
	case INTEL_PT_TRIG_IP: {
		...
		unsigned int trv = intel_pt_pkt_trigs(packet);
		bool ip = packet->type == INTEL_PT_TRIG_IP;
		bool mult = flags & INTEL_PT_TRIG_MULT;
		bool inst = flags & INTEL_PT_TRIG_INST;

		if (flags & INTEL_PT_TRIG_ICNTV)
			return snprintf(buf, buf_len,
					"%s IP:%d MULT:%d INST:%d Triggers 0x%x ICNT:%d",
					name, ip, mult, inst, trv, packet->count);
		return snprintf(buf, buf_len, "%s IP:%d MULT:%d INST:%d Triggers 0x%x",
				name, ip, mult, inst, trv);
		}
	default:
		break;
	}
