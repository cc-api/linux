...
{
	unsigned long trv = ptq->state->triggers;
	int trig_nr;
	int err;

	for_each_set_bit(trig_nr, &trv, INTEL_PT_MAX_TRIGGERS) {
...
static int intel_pt_synth_trig_pkt_sample(struct intel_pt_queue *ptq)
{
	...
	raw.flags        = ptq->state->trigger_flags;
	raw.triggers     = ptq->state->triggers;
	raw.reserved3    = 0;
