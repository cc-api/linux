...
static struct intel_pt_aux_output_opts {
	...
	{"on-event:trigger", 0xc008, false},
	{"on-event:trigger-pause", 0xe008, false},
	{"on-event:trigger-resume", 0xd008, false},
	{"on-event:trigger-noip", 0x8008, false},
	{"on-event:trigger-noip-pause", 0xa008, false},
	{"on-event:trigger-noip-resume", 0x9008, false},
...
static int intel_pt_configure_aux_output(struct perf_pmu *intel_pt_pmu, struct evlist *evlist)
{
	struct intel_pt_trigger_caps caps;
				      ...
				      struct record_opts *opts)
{
	...
	err = intel_pt_configure_aux_output(intel_pt_pmu, evlist);
	if (err)
		return err;
