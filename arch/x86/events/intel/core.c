...
static struct event_constraint intel_skt_event_constraints[] __read_mostly =
{
	...
	FIXED_EVENT_CONSTRAINT(0x003c, 1), /* CPU_CLK_UNHALTED.CORE */
	FIXED_EVENT_CONSTRAINT(0x0300, 2), /* pseudo CPU_CLK_UNHALTED.REF */
	FIXED_EVENT_CONSTRAINT(0x0073, 4), /* TOPDOWN_BAD_SPECULATION */
	FIXED_EVENT_CONSTRAINT(0x019c, 5), /* TOPDOWN_FE_BOUND */
	FIXED_EVENT_CONSTRAINT(0x02c2, 6), /* TOPDOWN_RETIRING */
	EVENT_CONSTRAINT_END
};
...
EVENT_ATTR_STR_HYBRID(topdown-retiring,      td_retiring_arl_h,
		      "event=0xc2,umask=0x02;event=0x00,umask=0x80;event=0xc2,umask=0x0", hybrid_big_small);
EVENT_ATTR_STR_HYBRID(topdown-bad-spec,      td_bad_spec_arl_h,
		      "event=0x73,umask=0x0;event=0x00,umask=0x81;event=0x73,umask=0x0", hybrid_big_small);
EVENT_ATTR_STR_HYBRID(topdown-fe-bound,      td_fe_bound_arl_h,
		      "event=0x9c,umask=0x01;event=0x00,umask=0x82;event=0x71,umask=0x0", hybrid_big_small);
EVENT_ATTR_STR_HYBRID(topdown-be-bound,      td_be_bound_arl_h,
		      "event=0xa4,umask=0x02;event=0x00,umask=0x83;event=0x74,umask=0x0", hybrid_big_small);
...
EVENT_ATTR_STR_HYBRID(mem-loads,     mem_ld_arl_h,
		      "event=0xd0,umask=0x5,ldlat=3;event=0xcd,umask=0x1,ldlat=3;event=0xd0,umask=0x5,ldlat=3", hybrid_big_small);
EVENT_ATTR_STR_HYBRID(mem-stores,    mem_st_arl_h,
		      "event=0xd0,umask=0x6;event=0xcd,umask=0x2;event=0xd0,umask=0x6", 			hybrid_big_small);
...
__init int intel_pmu_init(void)
{
	...
	case INTEL_FAM6_CLEARWATERFOREST_X:
	case INTEL_FAM6_CLEARWATERFOREST_D:
		x86_pmu.mid_ack = true;
		memcpy(hw_cache_event_ids, glp_hw_cache_event_ids,
		       sizeof(hw_cache_event_ids));
		memcpy(hw_cache_extra_regs, tnt_hw_cache_extra_regs,
	...
	case INTEL_FAM6_ARROWLAKE_H:
		...
		x86_pmu.hybrid_pmu = kcalloc(X86_HYBRID_SOC_NUM_PMUS,
					     sizeof(struct x86_hybrid_pmu),
					     GFP_KERNEL);
		if (!x86_pmu.hybrid_pmu)
			return -ENOMEM;
		static_branch_enable(&perf_is_hybrid);
		x86_pmu.num_hybrid_pmus = X86_HYBRID_SOC_NUM_PMUS;
		...
		pmu = &x86_pmu.hybrid_pmu[X86_HYBRID_PMU_SOC_ATOM_IDX];
		pmu->name = "soc_atom";
		pmu->cpu_type = get_hybrid_core_id();
		pmu->mid_ack = true;
		pmu->num_counters = x86_pmu.num_counters;
