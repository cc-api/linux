					...
					u64 stream_exinfo_pa)
{
	...
	ret = seamcall_retry(TDH_IDE_STREAM_CREATE, iommu_id, spdm_session_idx,
			       ...
			       u64 entry6, u64 entry7)
{
	 ...
	 */
	return seamcall_retry(TDH_DMAR_ADD, index, tdr_pa,
...
static inline u64 tdh_dmar_read(u64 index, struct tdx_module_output *out)
{
	 ...
	 * Output: R8-R15
	 */
	return seamcall_retry(TDH_DMAR_READ, index,
			      0, 0, 0, 0, 0, 0, 0, 0, 0, out);
}
				...
				u64 wait_desc_1, u64 wait_desc_2)
{
	 ...
	 */
	return seamcall_retry(TDH_IQINV_REQ, iommu_id, inv_type, inv_target,
