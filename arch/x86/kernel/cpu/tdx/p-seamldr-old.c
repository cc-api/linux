...
static int seamldr_info(phys_addr_t seamldr_info)
{
	...
	ret = seamcall_old(SEAMCALL_SEAMLDR_INFO, seamldr_info, 0, 0, 0, NULL);
	if (ret) {
...
int seamldr_install(phys_addr_t seamldr_params)
{
	...
	ret = seamcall_old(SEAMCALL_SEAMLDR_INSTALL, seamldr_params, 0, 0, 0, NULL);
	if (ret) {
		pr_err_ratelimited(
