...
bool __read_mostly enable_apic_timer_virt;
...
#ifdef CONFIG_X86_64
		/*
		 * FRED MSRs are per-cpu, however FRED CONFIG and STKLVLS
		 * MSRs are the same on all CPUs, thus they are initialized
		 * in vmx_set_constant_host_state() instead.
		 */
		if (cpu_feature_enabled(X86_FEATURE_FRED)) {
			     ...
			     struct vmx_capability *vmx_cap)
{
	 ...
	 */
	if (cpu_feature_enabled(X86_FEATURE_FRED) &&
	    !(basic_msr & (1u << (VMX_BASIC_NESTED_EXCEPTION_SHIFT - 32)))) {
...
static int vmx_set_guest_virt_timer(struct kvm_vcpu *vcpu, u16 vector)
{
	...
	/* This HACK is only for intel-next compatible with TDX */
	if(!enable_apic_timer_virt)
		return -EPERM;
