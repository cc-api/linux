...
/**
 * struct sdsi_spdm_state - SPDM session state
 *
...
static int spdm_exchange(struct sdsi_spdm_state *spdm_state,
...
static int spdm_get_capabilities(struct sdsi_spdm_state *spdm_state,
 ...
 */
static int spdm_start_hash(struct sdsi_spdm_state *spdm_state,
...
static int spdm_parse_algs(struct sdsi_spdm_state *spdm_state)
{
...
static int spdm_negotiate_algs(struct sdsi_spdm_state *spdm_state,
			       void *transcript, size_t transcript_sz)
{
...
static int spdm_validate_certificate(struct sdsi_spdm_state *spdm_state, u8 slot,
...
static int spdm_get_certificate(struct sdsi_spdm_state *spdm_state, u8 slot)
{
 ...
 */
static void spdm_create_combined_prefix(struct sdsi_spdm_state *spdm_state,
 ...
 */
static int spdm_verify_signature(struct sdsi_spdm_state *spdm_state, u8 *s,
 ...
 */
static size_t spdm_challenge_rsp_sz(struct sdsi_spdm_state *spdm_state,
...
static void spdm_reset(struct sdsi_spdm_state *spdm_state)
{
...
static void spdm_measurement_prepare(struct sdsi_spdm_state *spdm_state)
{
	kfree(spdm_state->desc);
	spdm_state->desc = NULL;
...
/**
 * sdsi_spdm_authenticate() - Authenticate device
 *
 * @spdm_state: SPDM session state
 *
...
EXPORT_SYMBOL_GPL(sdsi_spdm_authenticate);

static int spdm_start_mhash(struct sdsi_spdm_state *spdm_state)
{
	int rc;
...
static int __spdm_get_measurements(struct sdsi_spdm_state *spdm_state,
				   struct spdm_measurement_request *m)
{
	...
	if (m->attribute & MEASUREMENT_ATTR_SIGN) {
		...
		if (spdm_state->version == 0x10) {
			req_sz = offsetof(typeof(req), slot);
		} else {
			/* SlotIDParam */
...
/**
 * sdsi_spdm_get_measurements() - Get device measurements
 *
 * @spdm_state: SPDM session state
 * @m:
 *
...
EXPORT_SYMBOL_GPL(sdsi_spdm_get_measurements);

/**
 * sdsi_spdm_create() - Allocate SPDM session
 *
...
EXPORT_SYMBOL_GPL(sdsi_spdm_create);

/**
 * sdsi_spdm_destroy() - Destroy SPDM session
 *
 * @spdm_state: SPDM session state
 */
...
EXPORT_SYMBOL_GPL(sdsi_spdm_destroy);
