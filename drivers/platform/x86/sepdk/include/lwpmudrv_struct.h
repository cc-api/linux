...
typedef struct PMT_TOPOLOGY_DISCOVERY_LIST_NODE_S PMT_TOPOLOGY_DISCOVERY_LIST_NODE;
...
#define PMT_TOPOLOGY_DISCOVERY_LIST_topology_detected(x) ((x)->topology_detected)
...
#define PMT_TOPOLOGY_DISCOVERY_LIST_tile_oobmsm_deviceid(x, pmt_dev_index,     \
							 pmt_dev_tile_index)   \
	((x)->pmt_device[pmt_dev_index].pmt_tiles[pmt_dev_tile_index].oobmsm_deviceid)
#define PMT_TOPOLOGY_DISCOVERY_LIST_tile_pmt_endpoint_guid(x, pmt_dev_index,   \
							   pmt_dev_tile_index) \
	((x)->pmt_device[pmt_dev_index].pmt_tiles[pmt_dev_tile_index].pmt_endpoint_guid)
...
#define UNCORE_TOPOLOGY_INFO_pcidev_is_devno_funcno_valid(x, dev_index, devno, \
							  funcno)              \
	((x)->device[dev_index].pcidev[devno].func_info[funcno].valid ? TRUE : FALSE)
...
typedef struct UNCORE_DISCOVERY_DVSEC_CONFIG_NODE_S UNCORE_DISCOVERY_DVSEC_CONFIG_NODE;
...
typedef struct UNCORE_DISCOVERY_TABLE_LIST_NODE_S UNCORE_DISCOVERY_TABLE_LIST_NODE;
...
#define UNCORE_DISCOVERY_pci_ext_cap_id(x)      ((x)->dvsec_config_node.pci_ext_cap_id)
...
#define UNCORE_DISCOVERY_dvsec_bir_mask(x)      ((x)->dvsec_config_node.dvsec_bir_mask)
...
#define PLATFORM_TOPOLOGY_PROG_EXT_topology_device_distinct_buses(x, dev_index)   \
...
#define DRV_SETUP_INFO_process_exit_hook_unavailable(info)                     \
	((info)->u1.s1.process_exit_hook_unavailable)
#define DRV_SETUP_INFO_munmap_hook_unavailable(info)                           \
...
//      (module_id * threads_per_module + core_id * threads_per_core + thread_id)  * num_core_events    //thread offset
#define EMON_BUFFER_CORE_THREAD_OFFSET(package_id,                \
		num_entries_per_package, device_offset_in_package,\
		module_enum_supported, module_id,                 \
		threads_per_module, core_id,                      \
		threads_per_core, thread_id, num_core_events)     \
	(package_id * num_entries_per_package + device_offset_in_package +  \
		(module_enum_supported * module_id * threads_per_module + core_id * threads_per_core + thread_id) \
		* num_core_events)
...
//      (module_id * threads_per_module + core_id * threads_per_core + thread_id) * num_thread_events  + //thread offset
...
#define EMON_BUFFER_UNCORE_THREAD_POWER_EVENT_OFFSET(                           \
	...
	module_enum_supported, module_id, threads_per_module, core_id,          \
	threads_per_core, thread_id, num_unit_events, device_event_offset)      \
	(package_id * num_entries_per_package + device_offset_in_package +      \
		num_package_events + num_modules_per_package *                  \
		num_module_events + (module_enum_supported * module_id *        \
		threads_per_module + core_id * threads_per_core + thread_id)    \
		* num_unit_events  + device_event_offset)
