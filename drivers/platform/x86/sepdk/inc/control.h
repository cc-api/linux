...
#define GLOBAL_STATE_symbol_lookup_available(x) ((x).symbol_lookup_available)
...
struct CPU_STATE_NODE_S {
	...
	// are the previous values valid? (e.g., the first measurement does not have
	// a previous value for calculating the delta's
	DRV_BOOL last_p_state_valid;
	DRV_BOOL p_state_counting; // Flag to mark PMI interrupt from fixed event

 	// holds the data that is saved/restored during event multiplexing
	S64     *em_tables;
	U32      em_table_offset;
	...
	// holds PMU state (e.g., MSRs) that will be saved before and restored after collection
	U64     *pmu_state;
	S32      socket_master;
	S32      core_master;
	S32      thr_master;
...
struct MEM_TRACKER_NODE_S {
	// MAX number of elements in the array (default: MEM_EL_MAX_ARRAY_SIZE)
	U16         max_size;
	// number of elements available in this array
	U16         elements;
...
extern   U32                 *core_to_module_map;
extern   U32                 *core_to_dev_map;
extern   U32                 *core_to_phys_core_map;
extern   U32                 *core_to_thread_map;
extern   U32                 *cores_per_module;
extern   U32                 *threads_per_core;
extern   U32                  max_threads_per_core;
extern   U32                  max_cores_per_module;
extern   U32                  module_enum_supported;
...
extern VOID
CONTROL_Invoke_Cpu(S32 cpuid, VOID (*func)(PVOID), PVOID ctx);
