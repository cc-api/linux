...
#define SYS_Read_MSR(msr) SYS_Read_MSR_With_Status((msr), (NULL))

extern void SYS_Write_MSR_With_Status(U32 msr, U64 val, S32 *status);
#define SYS_Write_MSR(msr, val) SYS_Write_MSR_With_Status((msr), (val), (NULL))
...
#if defined(DRV_USE_RDPMC)
#define SYS_Read_PMC(ctr_addr, is_fixed_ctr) \
	SYS_Read_PMC_opt((ctr_addr), (is_fixed_ctr))
#else
#define SYS_Read_PMC(ctr_addr, is_fixed_ctr) SYS_Read_MSR((ctr_addr))
#endif
...
#define RDPMC_COUNTER_TYPE_BIT_SHIFT 30
#if defined(DRV_USE_RDPMC)
#define LFENCE_SERIALIZE() __asm__("lfence")
#else
#define LFENCE_SERIALIZE() {}
#endif
...
#define DRV_LOG_DEFAULT_LOAD_VERBOSITY \
...
#define DRV_LOG_DEFAULT_DETECTION_VERBOSITY \
	(DRV_LOG_DEFAULT_INIT_VERBOSITY)
#define DRV_LOG_DEFAULT_ERROR_VERBOSITY \
	(LOG_CHANNEL_MOSTWHERE | LOG_CONTEXT_ALL)
#define DRV_LOG_DEFAULT_STATE_CHANGE_VERBOSITY \
	(DRV_LOG_DEFAULT_INIT_VERBOSITY)
#define DRV_LOG_DEFAULT_MARK_VERBOSITY \
	(LOG_CHANNEL_MOSTWHERE | LOG_CONTEXT_ALL)
 ...
 */
extern char const ** UTILITY_Log_Category_Strings(void);
...
#define SEP_DRV_LOG_INCREMENT_NB_ACTIVE_INTERRUPTS()                           \
	{                                                                   \
		...
		__sync_fetch_and_add(&DRV_LOG_BUFFER_nb_interrupts(DRV_LOG()), \
				     1);                                       \
	}

#define SEP_DRV_LOG_DECREMENT_NB_ACTIVE_INTERRUPTS()                          \
	__sync_fetch_and_add(&DRV_LOG_BUFFER_nb_active_interrupts(DRV_LOG()), \
			     -1)
...
#define SEP_DRV_LOG_DISAMBIGUATE() \
	__sync_fetch_and_add(&DRV_LOG_BUFFER_disambiguator(DRV_LOG()), 1)
