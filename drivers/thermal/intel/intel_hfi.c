...
static int hfi_features_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "Capabilities:tP:%d EE:%d R:%d\n",
...
static int hfi_state_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "Timestamp\t%lld\n", *hfi_instance->local_table.timestamp);
	seq_puts(s, "\nPer-CPU data\n");
	...
	for (i = 0; i < hfi_features.nr_classes; i++) {
		...
		seq_printf(s, "%u\t", hdr_data->perf_updated);
		hfi_hdr++;
	}
	...
	for (i = 0; i < hfi_features.nr_classes; i++) {
		...
		seq_printf(s, "%u\t", hdr_data->ee_updated);
		hfi_hdr++;
	}
	...
	for_each_cpu(i, hfi_instance->cpus) {
		...
		void *data_ptr = hfi_instance->local_table.data +
...
/**
 * intel_hfi_enabled() - Check if intel_hfi driver enables HFI function.
 *
 * Return: True iff intel_hfi driver enables HFI function.
 */
bool intel_hfi_enabled(void)
{
	return hfi_instances;
}
				  ...
				  unsigned int nr_classes)
{
	...
	features->nr_table_pages = hfi_features.nr_table_pages;
	 ...
	 */
	features->hdr_size =
		round_up(features->class_stride * hfi_features.nr_classes, 8);
	features->cpu_stride =
		round_up(features->class_stride * hfi_features.nr_classes, 8);
	return 0;
}
