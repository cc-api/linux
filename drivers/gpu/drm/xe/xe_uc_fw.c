...
#define XE_GUC_FIRMWARE_DEFS(fw_def, mmp_ver, major_ver)			\
	fw_def(FALCONSHORES,	mmp_ver(  xe,	guc,	xe3pxp,	70, 6, 6))	\
	fw_def(LUNARLAKE,	mmp_ver(  i915,	guc,	lnl,	70, 6, 6))	\
	fw_def(BATTLEMAGE,	mmp_ver(  i915,	guc,	bmg,	70, 6, 6))	\
...
static int uc_fw_xfer(struct xe_uc_fw *uc_fw, u32 offset, u32 dma_flags)
{
	...
	ret = xe_mmio_wait32(gt, DMA_CTRL.reg, 0, START_DMA, timeout_us, &dma_ctrl,
			     false);
	if (ret)
